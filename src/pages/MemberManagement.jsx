...
const MemberManagement = () => {
    ...
    const { data: registrationsData, isLoading: registrationsLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const response = await axiosSecure.get(`/users/meals/my-registrations?startDate=${format(currentWeekStart, 'yyyy-MM-dd')}&endDate=${format(weekEnd, 'yyyy-MM-dd')}`);
        ...
        if (registered) {
            ...
            toast.promise(
                axiosSecure.post('/users/meals/register', {
                    userId, // Manager can register for any user
                    date: dateStr,
                    mealType
                }).then(() => refetch()),
                {
